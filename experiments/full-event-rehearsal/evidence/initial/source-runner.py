...
def main():
    p=argparse.ArgumentParser();p.add_argument('--output',type=Path,required=True);p.add_argument('--steps',type=int,default=1200);a=p.parse_args()
    ...
    dialogue=[r for sid in ids for r in rows_for(sources[sid],drafts[sid],meta)]
    ...
    for arm,replay in [('narrow',narrow),('full',full)]:
