...
class Config:
    ...
    socials: int = 0
    # The move a turn performs, as a typed id rather than a "# dispute:" line
    # of prompt text. Zero when the run predates typed moves.
    controls: int = 0
...
# The move a turn performs, matching cc_core_account.h's CcCoreControl one for
# one. Zero means absent, so an untyped row encodes as it did before.
CONTROL_IDS = {name: i + 1 for i, name in enumerate(
    ['open', 'answer', 'remark', 'affirm', 'dispute', 'hedge', 'attribute',
     'defer', 'settle', 'part', 'recall', 'muse', 'cite'])}
META_FIELDS = 17
...
def channels_for(model):
    ...
    return {'typed_stance': bool(getattr(config, 'voices', 0)),
            ...
            'typed_control': bool(getattr(config, 'controls', 0))}


def encode_row(tokenizer, row, context=512, slots=False, packet=False, conversation=False,
               typed_stance=False, situation=False, social=False, typed_control=False):
    ...
    if packet or conversation:
        ...
        # say: for spoken lines, # think: for internal thoughts. Typed, the move
        # rides the meta channel instead and the line is gone.
        if mind and not typed_control:
    ...
    n = len(tokens) - 1
    meta = [[0, 0, 0, 0, row.get('kind_id', 0) if i < len(prefix) else 0] + [0] * 12
            for i in range(n)]
    ...
    if social and (packet or conversation):
        ...
        for i in range(min(len(prefix), n)):
            meta[i][12:16] = ids
    if typed_control and (packet or conversation):
        # The move as an id, added at every prefix position, replacing the
        # "# dispute:" line whose cue the model had to attend back to.
        control_id = CONTROL_IDS.get(row.get('control', 'say'), 0)
        for i in range(min(len(prefix), n)):
            meta[i][16] = control_id
...
class Crownless(nn.Module):
    def __init__(self, config=Config(), mode='copy'):
        ...
        # The move, typed: the last conditioning that rode prompt text.
        self.controls = nn.Embedding(16, config.dim) if config.controls else None
    ...
    def hidden(self, tokens, meta, caches=None):
        ...
        if self.mode != 'text':
            ...
            if self.controls is not None and meta.shape[-1] > 16:
                x = x + (meta[..., 16] != 0).unsqueeze(-1) * self.controls(meta[..., 16])
