"""Tune the 5M core on the thirteen-act corpus: conversation and mind together.

Earlier runs trained one act family at a time and lost the other. This one
reads a corpus that already contains both, anchors the conversation half to the
shipped model by distillation, and admits a checkpoint only when it still
answers plain accounts and still holds a four-turn exchange without inventing
words. The gate reads generated text, never loss -- loss called a checkpoint
4.3x worse where its answers had actually improved.
"""
import argparse
import hashlib
import json
...
from crownless_conversation import approved_response, CONVERSATION_ACTS, MIND_ACTS
...
def evaluate(model, tokenizer, rows, rules, label, known=None):
    ...
    return {'label': label, 'count': len(scored),
...
def main():
    ...
    talk = [r for r in corpus['train'] if r['act'] in CONVERSATION_ACTS]
    ...
    manifest = {'source_commit': subprocess.check_output(['git', 'rev-parse', 'HEAD'], text=True).strip(),
                'source_hashes': {name: sha(Path(__file__).with_name(name)) for name in
                                  ('crownless_v2.py', 'crownless_conversation.py', 'train_crownless_acts.py')},
                ...
                'acts': {'conversation': CONVERSATION_ACTS, 'mind': MIND_ACTS},
    ...
    talk_validation = [encoded(tokenizer, r) for r in
                       [x for x in corpus['validation'] if x['act'] in CONVERSATION_ACTS][:244]]
    guard = [x for x in corpus['validation'] if x['act'] in CONVERSATION_ACTS][:args.guard_rows]
    ...
    if best_step == 0:
        ...
        if (args.output / 'candidate.pt').exists():
            ...
            report = {'rejected': True, 'reason': 'No checkpoint held both act families within tolerance',
        ...
        raise SystemExit('No checkpoint held both act families within tolerance')
    ...
    for label, rows in (('test', corpus['test'][:args.eval_rows]),
                        ...
                        ('bridge', [bridged(r, bridge_rng) for r in
                                    [x for x in corpus['test'] if x['act'] in CONVERSATION_ACTS][:args.eval_rows]]),
                        ('chat', chat)):
