...
/opt/zero-venv/bin/python -m unittest discover -s tests -p test_replication.py
/opt/zero-venv/bin/python -m unittest discover -s tests -p test_subword.py
/opt/zero-venv/bin/python scripts/train_subword.py --data data --output preflight --runs 5m-wide-256 --deadline "$DEADLINE" --small-tokens 16384 --report 2 --selection-windows 2 --final-windows 2
(
  while sleep 120; do
    if [ -d output ]; then
...
sync_pid=$!
/opt/zero-venv/bin/python scripts/score_subword_windows.py --models models --data data --output output/existing-window-rescore --device cuda --precision bf16
/opt/zero-venv/bin/python scripts/bootstrap_subword.py output/existing-window-rescore
/opt/zero-venv/bin/python scripts/run_subword_replication.py --data data --prefix-data prefix-data --extra extra --output output --deadline "$DEADLINE"
/opt/zero-venv/bin/python scripts/summarize_replication.py --output output --data data --prefix-data prefix-data --diagnostics diagnostics
kill "$sync_pid" || true
